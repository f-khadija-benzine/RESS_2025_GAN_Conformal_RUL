...
# ── Version stamp ────────────────────────────────────────────────────
# Bump this whenever detect_fpt or the labelling logic changes, so a stale
# copy in Colab is immediately obvious.
__version__ = "2.0-guarded-fpt"

print(f"[health_indicator] v{__version__} loaded  "
      f"(FPT: 5 consecutive x max(mu+3sigma, mu*1.10))")
...
class HealthIndicatorPipeline:
    ...
    def __init__(self, hi_method='simple_rms', smoothing_window=3,
        ...
        print(f"[HealthIndicatorPipeline] v{__version__} | "
              f"hi={hi_method} | fpt_consecutive={fpt_consecutive} | "
              f"fpt_min_relative_rise={fpt_min_relative_rise}")
