...
def detect_fpt(hi, n_consecutive=2):
    """Detect First Prediction Time using 2-sigma threshold.

    FPT = first time n_consecutive consecutive HI values exceed mu + 2*sigma,
    ...
    Returns:
        fpt_idx: index of FPT (or 0 if not detected)
    """
    # Compute baseline statistics from first 20% of data
    ...
    # If never detected, use 80% of signal as default FPT
    return int(len(hi) * 0.8)
...
class HealthIndicatorPipeline:
    ...
    def process_bearing(self, bearing_data, verbose=True):
        ...
        fpt_idx = detect_fpt(hi_sm, self.fpt_consecutive)
        ...
        if verbose:
            print(f"  FPT: t={fpt_idx} min | Acc: t={acc_idx} min | EOL: t={eol_idx} min")
